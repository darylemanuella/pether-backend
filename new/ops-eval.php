...
//    log_to_file("now running post-op");
    ...
    if($skip !==0)
        ...
        $result=array_slice($result,$skip,NULL,false);
//    log_to_file("${GLOBALS['ipaddr']} ${serial} DEBUG: ".print_r($result, true));
    if(count($result)==0){
//        log_to_file("${GLOBALS['ipaddr']} ${serial} DEBUG: No approver contacts");
        return;
    }
    ...
    if(NULL==$to){
        ...
        if(count($result)>0){
           // log_to_file('contact details:'.PHP_EOL. print_r($result, TRUE));
            $result=$result[0];
            if(count($result)>0)
                $to=$result[0]['p.claimsms'==$op?'telno'];
        }
        if(NULL==$to)
            send_error("Found no ". ('p.claimsms'==$op?'phone number'),
                $op, WS_ERR_PARAM);
    }
...
//    log_to_file('claims:'.PHP_EOL.print_r($data, FALSE));
    ...
    $claims=$data[1];
/*    if(count($claims)==0)
    send_error('There are no claims','',WS_ERR_UNK);
 */
    $meta=$data[0][0];
    $spname=$meta['spname'] ;
    ...
    // if('p.claimsms'==$op){
        ...
        require_once('../ws-utils/send-sms.php');
        $output="Dear".$lastname."a claim of." .$insurercost." from ".$spname."on".$transdate.
