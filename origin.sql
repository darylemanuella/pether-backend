FINER="fotang"@"%" PROCEDURE "get_claim_taints"(
...
as
begin
    call enrolutil.checkLoginStatus(p_connId,p_caller, 'get_claim_taints');

   if p_claimid is not null then
        select tc._claimid claimid, tc.taintid as id, tc.qty, tc.insurercost icost,tc.orig_qty, tc.orig_insurercost orig_icost,
        ...
        begin
            ...
            set v_sql =
                'select tc._claimid claimid, tc.taintid as id, tc.qty, tc.insurercost icost,tc.orig_qty, tc.orig_insurercost orig_icost,
                    status_ok_id is not null as accepted, tc.asof createdAt, userid2name(tc.createdBy) as createdBy
                    from tainted_claim_details tc join claim_details c on c._claimid=tc._claimid left join deleted_claims dcl on dcl._claimid=tc._claimid';
            if (p_caller='I') then
